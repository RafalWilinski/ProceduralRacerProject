...
{
    [CustomEditor (typeof(ColorGradingLookup))]
    class ColorGradingLookupEditor : Editor
    {
        ...
        public override void OnInspectorGUI () {
            ...
            if (tempClutTex2D == null) {
                t = AssetDatabase.LoadMainAssetAtPath(((ColorGradingLookup)target).basedOnTempTex) as Texture2D;
                if (t) tempClutTex2D = t;
            }

            Texture2D tex = tempClutTex2D;

            if (tex && (target as ColorGradingLookup).basedOnTempTex != AssetDatabase.GetAssetPath(tex))
            {
                EditorGUILayout.Separator();
                if (!(target as ColorGradingLookup).ValidDimensions(tex))
                {
                ...
                else if (GUILayout.Button ("Convert and Apply"))
                {
                    ...
                    (target as ColorGradingLookup).Convert(tex, path);
                }
            }

            if ((target as ColorGradingLookup).basedOnTempTex != "")
            {
                EditorGUILayout.HelpBox("Using " + (target as ColorGradingLookup).basedOnTempTex, MessageType.Info);
                t = AssetDatabase.LoadMainAssetAtPath(((ColorGradingLookup)target).basedOnTempTex) as Texture2D;
                if (t) {

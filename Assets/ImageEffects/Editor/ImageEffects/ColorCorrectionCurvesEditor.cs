...
{
    [CustomEditor (typeof(ColorGradingCurves))]
    class ColorGradingCurvesEditor : Editor {
        ...
        void ApplyCurves () {
            if (applyCurveChanges) {
                serObj.ApplyModifiedProperties ();
                (serObj.targetObject as ColorGradingCurves).gameObject.SendMessage ("UpdateTextures");
            }
        }

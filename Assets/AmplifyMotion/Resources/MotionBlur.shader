...
Shader "Hidden/Amplify Motion/MotionBlur" {
	...
	SubShader {
		...
		CGINCLUDE
			...
			half4 _EFLOW_BLUR_STEP;
			half2 _EFLOW_DEPTH_THRESHOLD;
			...
			half4 frag_mobile( v2f i ) : SV_Target
			{
				...
				half ref_isobj = ( id > 1 && color.a < 254 );
				...
				half sample_depth1 = SAMPLE_DEPTH_TEXTURE( _CameraDepthTexture, i.uv.xy + dir_step );
				...
				half2 depth_test = sample_depth > ( ref_depth.xx - _EFLOW_DEPTH_THRESHOLD.xx );
			...
			half4 frag_sm2( v2f i ) : SV_Target
			{
				...
				half4 depth_test = sample_depth > ( ref_depth.xxxx - _EFLOW_DEPTH_THRESHOLD.xxxx );
			...
			half4 frag_sm3( v2f i ) : SV_Target
			{
				...
				half4 diff_testA = diffA < _EFLOW_DEPTH_THRESHOLD.xxxx;
				half4 diff_testB = diffB < _EFLOW_DEPTH_THRESHOLD.xxxx;
				half4 sample_testA = diff_testA - diff_testA * saturate( diffA * _EFLOW_DEPTH_THRESHOLD.yyyy );
				half4 sample_testB = diff_testB - diff_testB * saturate( diffB * _EFLOW_DEPTH_THRESHOLD.yyyy );

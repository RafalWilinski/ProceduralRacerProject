...
namespace ScionEngine
{
	...
	{
		public const float FilmWidth = 70.0f; 
		...
		private RenderTexture DownsampleTexture(RenderTexture renderTex, float energyNormalizer)
		{
			...
			{
				size = size / 2 + size % 2;
				RenderTexture downsampled = RenderTexture.GetTemporary(size, size, 0, renderTex.format);
				downsampled.filterMode = FilterMode.Bilinear;
				downsampled.wrapMode = TextureWrapMode.Clamp;

				if (firstIteration == true) 
				{
					m_virtualCameraMat.SetFloat("_EnergyNormalizer", energyNormalizer);
		...
		{
			mat.SetTexture("_VirtualCameraResult", m_currentResult2);
		}
		...
		public void CalculateVirtualCamera(CameraParameters cameraParams, RenderTexture textureToDownsample, float halfResWidth, 
		                                   float tanHalfFoV, float energyNormalizer, float focalDistance, bool isFirstRender)
		{			
			...
			RenderTexture downsampledScene = DownsampleTexture(textureToDownsample, energyNormalizer);

...
{	   	
	...
	{		
		...
		SCION_UNROLL for (int k = 0; k < 33; k++)
		{
	...
	{		
		...
		SCION_UNROLL for (int j = 0; j < 4; j++)
		{
			...
			float4 sample = tex2Dlod(_DepthOfFieldTexture, float4(sampleUV[j], 0.0f, 0.0f));
			
			upsampledDoF = upsampledDoF + sample * weight; 
		...
		//return tiledNeighbourhood.xxxx;
		
		float searchRadiusFactor = saturate(tiledNeighbourhood * 0.5f);
		float lerpFactor = saturate((searchRadiusFactor * upsampledDoF.w));		
	...
	Category 
	{
		Subshader 
		{
			...
			Pass 
			{
				...
				#pragma multi_compile SC_DOF_FOCUS_MANUAL SC_DOF_FOCUS_MANUAL_RANGE SC_DOF_FOCUS_CENTER		
				ENDCG	
			}
			Pass 
			{
			...
			Pass 
			{
				...
				#pragma multi_compile SC_EXPOSURE_AUTO SC_EXPOSURE_MANUAL	
				#pragma multi_compile SC_DOF_FOCUS_MANUAL SC_DOF_FOCUS_MANUAL_RANGE SC_DOF_FOCUS_CENTER		
				#pragma multi_compile SC_DOF_MASK_OFF SC_DOF_MASK_ON		
				ENDCG	
			}
			Pass 
			{

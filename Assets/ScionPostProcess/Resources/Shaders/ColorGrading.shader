...
{	
	...
	{
		float3 sample = tex2Dlod(_MainTex, float4(i.uv, 0.0f, 0.0f)).xyz;
	...
	float4 ConvertChromatica(v2f i) : SV_Target0
	{
		...
		float3 finalColor = lerp(sample0, sample1, frac(blue));
	...
	float4 ConvertUnity(v2f i) : SV_Target0
	{
		float2 uv = i.uv;
		uv.y = 1.0f - uv.y;
		float3 sample = tex2Dlod(_MainTex, float4(uv, 0.0f, 0.0f)).xyz;
		return float4(sample, 1.0f);
	}

#ifndef VACUUM_CURVEDWORLD_DEFINES_CGINC
#define VACUUM_CURVEDWORLD_DEFINES_CGINC
...
#define V_CW_BEND(v)  half4 mv = mul(UNITY_MATRIX_MV, v); \
				      half zOff = min(0, mv.z + V_CW_CAMERA_BEND_OFFSET); \
				      zOff = zOff * zOff * 0.001; \
				      half xOff = max(0, abs(mv.x) - V_CW_Z_BEND_BIAS) * sign(mv.x); half4 pos = mv; \
				      pos.xy += half2(V_CW_Y_BEND_SIZE * zOff, V_CW_X_BEND_SIZE * zOff + (xOff * xOff * V_CW_Z_BEND_SIZE) * 0.001);	o.pos = mul(UNITY_MATRIX_P, pos);
